...
export const img = ()  => src(`${path.img.root}/**/*.{png,jpg,jpeg,svg}`)
  .pipe(plumber(notify.onError({
    title: 'IMG',
    message: 'Error: <%= error.message %>'
  })))
...
export const images = ()  => src(`${path.images.root}/**/*.{png,jpg,jpeg,svg}`)
  .pipe(plumber(notify.onError({
    title: 'IMAGES',
    message: 'Error: <%= error.message %>'
  })))
...
export const server = () => {
  ...
  watch(`${path.images.root}/**/*.{png,jpg,jpeg,svg}`, images).on('change', bs.reload);
  watch(`${path.img.root}/**/*.{png,jpg,jpeg,svg}`, img).on('change', bs.reload);
};
